        ...
        try:
            ...
            try:
                ...
                if not row_data and columns_count > 0:
                    ...
                    # Update upload status to indicate file is processed (header-only, no data rows)
                    if upload_id:
                        mongodb_service.update_upload_status(
                            upload_id=upload_id,
                            status="processed",
                            metadata={
                                ...
                                "note": "File processed successfully with headers only (no data rows)"
                            }
                        )
                        logger.info(f"✅ Updated upload status to 'processed' for header-only file, upload_id={upload_id}")
                    
                    # Don't process further - file has no data rows to save
                    return
                    ...
                    if headers_count > 0:
                        ...
                        if normalized_headers:
                            try:
                                ...
                                # Update upload status to indicate file is processed (header-only) with parse error
                                # Headers were successfully extracted despite parse error, so mark as processed
                                if upload_id:
                                    mongodb_service.update_upload_status(
                                        upload_id=upload_id,
                                        status="processed",
                                        metadata={
                                            ...
                                            "processed_at": datetime.utcnow(),
                                            "note": "File processed with headers extracted (parse error occurred but headers were saved)"
                                        }
                                    )
                                    logger.info(f"✅ Updated upload status to 'processed' for header-only file with parse error, upload_id={upload_id}")
